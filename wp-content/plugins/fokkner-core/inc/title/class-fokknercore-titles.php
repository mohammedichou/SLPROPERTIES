...
class FokknerCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'fokkner_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
	...
	function include_layouts() {
		...
		foreach ( glob( FOKKNER_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( FOKKNER_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = fokkner_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'fokkner_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'fokkner_filter_title_template' : 'fokkner_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = fokkner_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = fokkner_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = fokkner_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
	...
	function add_inline_styles( $style ) {
		...
		$height                   = fokkner_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = fokkner_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'fokkner_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'fokkner_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = fokkner_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = fokkner_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = fokkner_core_get_page_title_image_params();
		...
		$title_color = fokkner_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
FokknerCore_Titles::get_instance();

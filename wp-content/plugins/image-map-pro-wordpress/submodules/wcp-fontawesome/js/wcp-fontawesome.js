;(function ($, window, document, undefined) {
    ...
    $.wcpFontawesomeUI = function() {
        var html = '';

        html += '<div id="icons-ui-wrap">';

        html += '	<div id="icon-search-wrap">';
        html += '		<i class="fa fa-search"></i>';
        html += '		<input type="text" id="input-icon-search" placeholder="Search Icons">';
        html += '	</div>';
        
        html += '	<div id="icon-container" style="max-height: '+ ($(window).height() - 330) +'px;">';
        
        var l = fontAwesome.length;
        var categories = [];

        for (var i=0; i<l; i++) {
            var iconHTML = '';
            var filter = '';
            if (fontAwesome[i].filter) {
                for (var j=0; j<fontAwesome[i].filter.length; j++) {
                    filter += ' ' + fontAwesome[i].filter[j];
                }
            }
            
            iconHTML += '<div class="fontawesome-icon-wrap '+ filter +'" data-fontawesome-id="'+ fontAwesome[i].id +'">';
            iconHTML += '	<i class="fa fa-2x fa-'+ fontAwesome[i].id +'"></i>';
            iconHTML += '	<span class="icon-name">'+ fontAwesome[i].name.toLowerCase() +'</span>';
            iconHTML += '	<span class="search-attachment"></span>';
            iconHTML += '</div>';
            iconHTML += '';

            if (!categories[fontAwesome[i].categories[0]]) categories[fontAwesome[i].categories[0]] = [];
            categories[fontAwesome[i].categories[0]].push(iconHTML);
        }

        for (category in categories) {
            html += '<div class="category-title-wrap active"><i class="fa fa-caret-down fa-2x"></i><i class="fa fa-caret-right fa-2x"></i><h1>'+ category +'</h1></div>';
            html += '<div>';
            
            for (var i=0; i<categories[category].length; i++) {
                html += categories[category][i];
            }
    ...
    $.wcpFontawesomeSearch = function(str) {
        var str = $('#input-icon-search').val();
		$('.icon-name-underlined').contents().unwrap();

		if (str.length > 2) {
			$('.category-title-wrap').hide();
			$('.fontawesome-icon-wrap').hide();
			$('.search-attachment').hide()
			...
			for (var i=0; i<l; i++) {
				...
				// match name
				if (fontAwesome[i].name.search(r) != -1) {
					matched = true;
					$('[data-fontawesome-id="'+ fontAwesome[i].id +'"]').show();
				}

				// match id
				if (fontAwesome[i].id.search(r) != -1) {
					matched = true;
				}
				...
				if (fontAwesome[i].filter) {
					for (var j=0; j<fontAwesome[i].filter.length; j++) {
						if (fontAwesome[i].filter[j].search(r) != -1) {
							matched = true;
							matchedFilter = fontAwesome[i].filter[j];
							break;
						}
					}
				}
				...
				if (matched) {
					$('[data-fontawesome-id="'+ fontAwesome[i].id +'"]').show().parent().show().prev().show().addClass('active');
					if (str.length > 2 && matchedFilter.length > 0) {
						$('[data-fontawesome-id="'+ fontAwesome[i].id +'"] .search-attachment').show().html(' (' + matchedFilter + ')');
					}

					var html = $('[data-fontawesome-id="'+ fontAwesome[i].id +'"] .icon-name').html();
					$('[data-fontawesome-id="'+ fontAwesome[i].id +'"] .icon-name').html(html.replace(r, '<span class="icon-name-underlined">'+ str +'</span>'))
				}
			}

			
		} else {
			$('.category-title-wrap').show();
			$('.fontawesome-icon-wrap').show();
			$('.search-attachment').hide()
		}
    }

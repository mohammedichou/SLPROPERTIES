;(function ($, window, document, undefined) {
    ...
    $.wcpTourSimpleRegister = function(options) {
        var t = new WCPTourSimple(options);
        tours[options.name] = t;
    }
    $.wcpTourStart = function(tourName) {
        tours[tourName].start();
    }
    $.wcpTourEnd = function(tourName) {
        tours[tourName].endTour();
    }
    $.wcpTourDisable = function(tourName) {
        tours[tourName].disableTour();
    }
    ...
    function WCPTourSimple(options) {
        this.id = Math.floor(Math.random() * 99999) + 1;
        this.options = options;
        this.currentStep = -1;
        this.root = undefined;
    }
    WCPTourSimple.prototype.start = function() {
        var html = '';

        html += '<div id="wcp-tour-'+ this.id +'" class="wcp-tour-root-simple">';
        html += '   <div class="wcp-tour-background-simple">';
        html += '   </div>';

        // Content

        html += '   <div class="wcp-tour-simple-container" style="width: '+ this.options.width +'px; height: '+ this.options.height +'px;">';
        html += '       <div class="wcp-tour-simple-slide-content"></div>';
        html += '       <div class="wcp-tour-simple-button" id="wcp-tour-simple-button-skip">Skip</div>';
        html += '       <div class="wcp-tour-simple-button" id="wcp-tour-simple-button-done">Done</div>';
        html += '       <div class="wcp-tour-simple-click-anywhere">Click anywhere to continue</div>';
        html += '       <div class="wcp-tour-simple-nav">';

        for (var i=0; i<this.options.slides.length; i++) {
            var active = '';
            if (i == 0) active = 'wcp-tour-simple-nav-circle-active';

            html += '<div class="wcp-tour-simple-nav-circle" data-index="'+ i +'"></div>';
        }
        ...
        this.events();
        this.presentSlide(0);
        localStorage[this.options.name] = 'seen';
    };
    WCPTourSimple.prototype.events = function() {
        var self = this;

        $(document).off('mouseup.wcotoursimple');
        $(document).on('mouseup.wcotoursimple', function(e) {
            // Clicked on nav circle?
            if ($(e.target).hasClass('wcp-tour-simple-nav-circle')) {
                var index = $(e.target).data('index');
                self.presentSlide(index);
            } else if ($(e.target).hasClass('wcp-tour-simple-button')) {
                // Clicked on skip/done?
                self.endTour();
            } else {
                // If else, next slide
                self.nextSlide();
    ...
    WCPTourSimple.prototype.nextSlide = function() {
        this.presentSlide(this.currentStep + 1);
    };
    WCPTourSimple.prototype.presentSlide = function(slideIndex) {
        if (slideIndex > this.options.slides.length - 1) {
            // slideIndex = this.options.slides.length - 1;
            this.endTour();
            return;
        }
        if (slideIndex < 0) {
        ...
        // Update content
        this.root.find('.wcp-tour-simple-slide-content').html(this.options.slides[this.currentStep].html);

        // Update nav circles
        this.root.find('.wcp-tour-simple-nav-circle').removeClass('wcp-tour-simple-nav-circle-active');
        this.root.find('.wcp-tour-simple-nav-circle[data-index="'+ this.currentStep +'"]').addClass('wcp-tour-simple-nav-circle-active');

        // Update buttons
        if (this.currentStep == this.options.slides.length - 1) {
            // Show done
            $('#wcp-tour-simple-button-skip').hide();
            $('#wcp-tour-simple-button-done').show();
        } else {
            // Show skip
            $('#wcp-tour-simple-button-skip').show();
            $('#wcp-tour-simple-button-done').hide();
        }
    };
    WCPTourSimple.prototype.endTour = function() {
        $('#wcp-tour-'+ this.id).remove();
        localStorage[this.options.name] = 'seen';
        $.wcpTourEventFinished(this.options.name);
    };
    WCPTourSimple.prototype.disableTour = function() {
        localStorage[this.options.name] = 'seen';
    };
    WCPTourSimple.prototype.isFinished = function() {
        if (localStorage[this.options.name] == 'seen') return true;
